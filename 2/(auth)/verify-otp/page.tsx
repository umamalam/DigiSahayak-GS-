...
export default function VerifyOTPPage() {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const res = await fetch('/api/auth/resend-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ emailOrPhone: email, sendVia: method }),
      });
  ...
  const displayEmail = method === 'email' ? email : `+91 ${'X'.repeat(email.length - 2)}${email.slice(-2)}`;
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-600 text-sm mt-3">
            Enter the 6-digit code sent to your registered {method === 'email' ? 'email address' : 'mobile number'}
          </p>
